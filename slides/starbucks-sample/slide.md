...
完成予定時刻を店舗の実測混雑データから算出し、アプリ側の表示を**実態に合わせる**。
...
3店舗にセンサーを設置し、時間帯別の受け取り待ち時間を実測します。現行の予定時刻との乖離を定量化します。
...
3. **予定時刻の精度**: 実測データが少ない店舗では従来より悪化する可能性
...
店舗ごとの実測待ち時間を返し、アプリ側が完成予定時刻を再計算します。
