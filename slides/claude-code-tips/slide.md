...
## 意図と乖離した場合は、指示を重ねず以前の状態へ巻き戻す

<div>

### 指示の追加によって意図のずれが拡大する例

```text
指示者「議事録を作成してください」
モデル「議事録を作成しました」
指示者「表現が過度に堅苦しいので、もう少し柔らかいトーンに調整してください」
モデル「親しみやすい表現に変更しました（過度にくだけた文体になる）」
指示者「取引先向けの公式文書なので、くだけすぎです。適切な敬語を保ってください」
（追加の修正指示を重ねるほど履歴が増加し、意図がさらにぶれやすくなる）
```
...
// ここでエスケープキーを 2 回押す（または /rewind を実行）
// 初期の指示時点までセッションを巻き戻す

指示者「取引先向けの会議議事録を作成してください。
　　　　丁寧な敬語を基本とし、以前作成した
　　　　`meeting-2026-05-01.md` と同じトーンで
　　　　まとめてください」
モデル「要件に沿った適切なトーンで作成しました」
```
